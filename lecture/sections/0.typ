...
#slide[
  - 那么rust是如何做的呢？
  #image("../images/3.svg", width: 100%, alt: "3")
]
...
#slide[
  ...
  - Rustfmt：统一的代码格式化工具，解决了团队协作中关于代码风格的无尽争论。

  - Clippy：一个非常有用的代码静态分析工具，能提供大量的代码改进建议，帮助你写出更地道、更高效的 Rust 代码。

  - #link("https://rustlings.rust-lang.org/")[🦀 Rustlings ]
...
#slide[
  在 Rust 中，&str (字符串切片) 是通用的、只读的字符串“视图”，它像一个灵活的“取景框”，可以用来观察任何字符串数据；

  而 String 和字符串字面量则是这个视图可以指向的两种主要数据“来源”。

  String 是动态创建、内容可以被修改的“可编辑文档”，它拥有数据的所有权，并能随时生成 &str 视图供他人读取。

  而字符串字面量则是一种特殊的、直接编译进程序的“石碑铭文”，它本身就是一种特殊的 &str，代表了程序自带的、生命周期为永久 ('static) 的不可变文本。
]

#slide[
  Unicode 和 UTF-8 是什么？

  想象一下你要制定一个全球通用的“字符身份证系统”。

  Unicode 的本质是一个标准，一个字符集（Character Set）。

  它的宏伟目标是为人类历史上出现过的每一个字符、符号（包括数学符号、表情符号等）都分配一个全球唯一的数字编号。这个编号就叫做“码点”（Code Point）。

  - 它做什么？ 它只负责定义“是什么”的映射关系。
    - 比如，它规定字母 `A` 的“身份证号”是 `U+0041`。
    - 汉字 `你` 的“身份证号”是 `U+4F60`。
    - 表情 `🦀` 的“身份证号”是 `U+1F980`。
  - 它不做什么？
    - Unicode 本身不规定这个“身份证号”应该如何在计算机里存储为二进制字节。它是一个抽象的规范，像一本巨大的字典。

  UTF-8：如何把“身份证号”写入电脑

  UTF-8 的本质是一种编码规则（Character Encoding），是 Unicode 标准的一种具体实现方式。

  它的任务是，接收一个 Unicode 码点（比如 `U+4F60`），然后按照一套规则，把它转换成计算机硬盘上或网络上传输的字节序列。

  - 它做什么？ 它负责定义“怎么存”的技术细节。
  - 核心特点：可变长度编码。它会根据字符的不同，使用 1 到 4 个字节来表示：
    - 1 字节：存储所有英文字母、数字和常见符号（完全兼容 ASCII）。
    - 2 字节：存储拉丁文、希腊文等。
    - 3 字节：存储绝大多数常用汉字、日文、韩文等。例如，`你` (U+4F60) 被存为 `E4 BD A0` 这三个字节。
    - 4 字节：存储不常用的字符和大部分表情符号。

  总结一下区别：

  - Unicode 是一个标准，定义了 “字符 -\> 数字” 的映射。
  - UTF-8 是一种编码，定义了 “数字 -\> 字节序列” 的存储方式。

  -----

]
#slide[
  Rust 在语言设计层面就将对 Unicode 的支持放在了核心位置，并做出了非常明确、安全的选择。

  核心原则：字符串 (`String`, `&str`) 必须是有效的 UTF-8

  这是 Rust 文本处理的基石。一个 `String` 类型在内部就是一个 `Vec<u8>`（一个字节的动态数组），但 Rust 语言强制保证这里面的字节序列必须是合法的 UTF-8 编码。

  Rust 如何使用它们？

  Rust 通过两种不同的类型来体现 Unicode 和 UTF-8 的关系：


  - `char` 类型：代表一个 Unicode 标量值。它的大小固定为 4 个字节，直接对应 Unicode 的“码点”概念。这使得任何单个字符（包括表情符号）都可以用一个 `char` 来表示。
  - `String` 和 `&str` 类型：代表一个 UTF-8 编码的字符串。它们内部是字节序列，所以一个 `char` 在字符串中可能占用 1 到 4 个字节。
...
#slide[
  ...
  ```rust
      ...
      // 1. .len() 获取的是字节长度，体现了 UTF-8 的存储方式；"你"(3) + "好"(3) + "🦀"(4) = 10 字节
      println!("字节长度: {}", s.len()); // 输出: 10
      // 2. .chars().count() 获取的是 char 的数量，体现了 Unicode 的字符视角； '你', '好', '🦀' 这 3 个 Unicode 字符
      println!("字符数量: {}", s.chars().count()); // 输出: 3
      // 3. 遍历 .chars() 可以安全地得到每一个 Unicode 字符 (char)
      for c in s.chars() {
          print!("{} ", c); // 输出: 你 好 🦀
      }
  ```]
#slide[
  Rust 为什么这么做？: Rust 做出这种设计的背后，是其核心理念——安全（Safety） 和 性能（Performance） 的体现。

  - 原因一：保证内存安全和数据正确性（杜绝乱码）
    这是最重要的原因。很多语言允许你通过索引 `s[i]` 直接访问字符串的某个字节。但在 UTF-8 中，这样做是极其危险的，因为你很可能只取到一个多字节字符的一部分，从而产生一个无效的、损坏的字符（即“乱码”）。

    Rust 在编译层面就禁止了这种不安全的操作，从根本上杜绝了此类错误。

    ```rust
    let s = "你好";
    // let c = s[0]; // 这行代码无法通过编译！错误：`str` 不能被索引
    ```
    这种设计强制开发者必须使用 `.chars()` 等安全的方法来处理文本，确保你得到的永远是有效的 Unicode 字符。
]
#slide[
  - 原因二：高效的存储和处理
    UTF-8 是一种非常高效的编码。对于以英文为主（在编程和网络协议中很常见）的文本，它和 ASCII 一样紧凑，每个字符只占 1 字节。同时它又能无缝支持所有其他语言。

    Rust 的 `&str` 类型本质上是一个 `&[u8]`（字节切片），这是一种非常轻量级和高性能的数据结构，可以直接在内存中进行操作而无需额外分配。

  - 原因三：提供清晰、无歧义的 API
    Rust 的 API 强迫你思考你的真实意图：

    - 你是想处理原始字节吗？用 `.bytes()`。
    - 你是想处理 Unicode 字符吗？用 `.chars()`。
    - 你是想处理用户看到的“视觉字符”（例如 `e` + `´` 组成的 `é`）吗？可以使用 `unicode-segmentation` 这个第三方库。

    这种清晰的划分避免了其他语言中因 API 模糊而导致的各种隐蔽 bug。

  总结

  Rust 的选择可以概括为：在内部使用高效的 UTF-8 字节序列进行存储和传输，同时在 API 层面提供安全的、面向 Unicode 字符的抽象。 通过在语言级别强制实施这些规则，Rust 为开发者构建了一个健壮的文本处理环境，从源头上避免了与编码相关的常见陷阱，让你能够自信地编写支持全球化多语言的程序。
